...
class BrowserErrorController:
    ...
    # エラーメッセージのパターン
    ERROR_PATTERNS = {
        "bot": ["ボット", "自動化", "不正なアクセス", "ReCAPTCHA"],
        "incorrect": ["ユーザーID", "パスワード", "正しくありません", "認証に失敗"],
        "expired": ["有効期限", "期限切れ", "セッション", "タイムアウト"],
        "maintenance": ["メンテナンス", "システム停止", "一時的に利用できません"],
        "network": ["ネットワーク", "接続エラー", "通信に失敗"],
        "locked": ["アカウントロック", "ロックされています", "一時停止"],
    ...
    def login(self, account: M_Account) -> ErrorResult:
        """ログインエラーのハンドリング"""
        error_message = self._get_error_message(self.LOGIN_ERROR_MESSAGE)
